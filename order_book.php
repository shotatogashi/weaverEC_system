...
echo "ファイル名：".$file_name.", ".$csv_name."<br />\n";
...
if (count($r_excluded_sample_order)) echo "サンプル注文のため除外：".implode(',', $r_excluded_sample_order)."<br />\n";
if (count($r_excluded_pending_order)) echo "サブステータスが保留のため除外：".implode(',', $r_excluded_pending_order)."<br />\n";

// 書き込み済み注文を削除
foreach ($r_csv_contents as $key => $order_num) {
	if (array_key_exists($order_num, $order_info)) {
		unset($order_info[$order_num]);
		echo "処理済み注文ID：".$order_num."<br />\n";
	}
}
...
echo "追記するデータの先頭40バイト：". mb_substr($new_contents, 0, 40)."<br />\n";
echo "追記するCSVの先頭40バイト：". mb_substr($new_csv_contents, 0, 40)."<br />\n";
echo "追記します<br />\n";
